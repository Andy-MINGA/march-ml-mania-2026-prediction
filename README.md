# march-ml-mania-2026-prediction
My first Kaggle March ML Mania competition (Brier score: 0.1698). Covers feature engineering, model optimization, and lessons learned for improving probabilistic predictions.
...
$$BS = \underbrace{\text{Reliability}}{\text{calibration error}} - \underbrace{\text{Resolution}}{\text{discrimination}} + \underbrace{\text{Uncertainty}}_{\text{irreducible}}$$
...
Margin of victory multiplier — $K_{\text{eff}} = K \cdot (1 + 0.04 \cdot \Delta)$, capped at $3K$. Winning by 30 is more informative than winning by 1.
Home court adjustment — 100-point bonus in the expected score calculation. Without this, home wins inflate ratings in a way that makes road wins look more impressive than they are.
Mean reversion between seasons — $r_t^{(s)} = 0.70 \cdot r_{\text{end}}^{(s-1)} + 0.30 \times 1500$. Models roster turnover. A dominant team three years ago with a completely different roster shouldn't carry that Elo forward.
...
---
Key Lessons
Calibration dominates on small datasets. Fixing the Reliability term improved the Brier score more than any single model change.
External data beats model complexity. KenPom contributed an estimated 15–20% improvement — more than adding four extra models.
Fewer models is better with 602 samples. Going from 5 to 3 models eliminated correlated noise and directly improved the Reliability component.
Temporal CV is not optional. Standard K-fold would have mixed same-season features across folds, inflating CV scores and producing incorrect model selection.
Feature engineering beats hyperparameter tuning. Time-decay weights, Bradley–Terry, conference strength, and coach continuity contributed signal that no amount of Optuna trials could conjure from nothing.
---
Tools
...
Open `march_mania_final.ipynb` in Google Colab.
Run Cell 1 immediately (anti-timeout).
Install libraries (Cell 2) then import (Cell 3).
Set your KenPom credentials in Cell 4, or set `USE_KENPOM = False` to skip.
Upload all Kaggle competition CSVs plus `vegas_odds_2026.csv` in Cell 5.
Run all cells sequentially. Total runtime: ~30–40 minutes on free Colab.
Download `submission_final_combined_2026.csv` from Cell 18 output.
Upload to Kaggle and manually select as active submission.
